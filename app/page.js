...
import './styles.css'
import { captionPromt, checkServer, getCaption, getEmotionCaption, getQuestions, logout, regenerateCaption, storeData, validateUser } from './services/caption.service';
import _, { forEach } from "lodash"
import { usePathname, useRouter } from 'next/navigation'
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { LuRefreshCcw, LuLayoutDashboard, LuLayers, LuSettings2, LuLogOut, LuSubtitles } from 'react-icons/lu';
...
export default function Home() {
  ...
  const handleChange = event => {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      getCaption(formData).then(async (response) => {
        setEmotionCaption(response.data["image-description"])
  ...
  const extractQuestions = (inputString) => {
    // Regular expression to match sentences starting with "what", "how", "when", etc. and ending with a question mark
    const questionRegex = /\b(?:what|how|when|where|why|which|who|whom|whose)\b.*\?/gi;

    // Match all the questions in the input string
    const matches = inputString.match(questionRegex);
    matches.splice(3)
    // Return the array of matched questions
    return matches || [];
  }
